...
def get_parser():
    ...
    parser.add_argument("--config",
                        type=str,
                        default='config/REAL/camera_real.yaml',
...
def init():
    ...
    log_dir = os.path.join("log_orig_res", exp_name)
...
if __name__ == "__main__":
    ...
    if not cfg.only_eval:
        ...
        # checkpoint = torch.load(os.path.join(cfg.ckpt_dir, 'epoch_' + str(cfg.test_epoch) + '.pt'))
        checkpoint = os.path.join(cfg.ckpt_dir, 'epoch_' + str(cfg.test_epoch) + '.pt')
        
        logger.info("=> loading checkpoint from path: {} ...".format(os.path.join(cfg.ckpt_dir, 'epoch_' + str(cfg.test_epoch) + '.pt')))
