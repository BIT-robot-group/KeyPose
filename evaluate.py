...
from utils.util import load_depth, get_bbox, compute_mAP, plot_mAP, draw_bboxes,draw_detections
...
parser.add_argument('--result_dir', type=str, default='', help='resume from saved model')

opt = parser.parse_args()

mean_shapes = np.load('data/mean_shapes.npy')

assert opt.data in ['val', 'real_test']
if opt.data == 'val':
    result_dir = 'results/val/test'
    file_path = 'camera/val_list_all.txt'
    cam_fx, cam_fy, cam_cx, cam_cy = 577.5, 577.5, 319.5, 239.5
else:
    # result_dir = 'results/real/test'
    result_dir = opt.result_dir
    file_path = 'real/test_list_all.txt'
...
def evaluate():
    ...
    # result_pkl_list = glob.glob(os.path.join(result_dir, '*_color.pkl'))
    ...
    for pkl_path in result_pkl_list:
        ...
        # draw_detections(img, "vis_results", opt.data, img_path_parsing[-1], real_intrinsics, \
        #         result['pred_RTs'], result['pred_scales'], result['pred_class_ids'], result['gt_RTs'], \
        #         result['gt_scales'], result['gt_class_ids'], result['nocs_RTs'], result['nocs_scales'], \
        #         result['nocs_class_ids'], draw_nocs=False)
...
def visualize():
    ...
    for path in tqdm(img_list):
        ...
        # load NOCS result
        nocs_pkl_path = os.path.join('/media/yusheng/G/NOCS/results/nocs_results', opt.data, 'results_{}_{}_{}.pkl'.format(
            opt.data.split('_')[-1], img_path_parsing[-2], img_path_parsing[-1]))
        ...
        # print(img_path)
        draw_detections(img, "log/camera_real/vis", opt.data, i,
             real_intrinsics, result['pred_RTs'], result['pred_scales'], result['pred_class_ids'], 
             result['gt_RTs'], result['gt_scales'], result['gt_class_ids'], result['nocs_RTs'],
              result['nocs_scales'], result['nocs_class_ids'], draw_nocs=False)
        i += 1 
