...
def test_func(model, dataloder, save_path_):
    ...
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            test_results = False
            if test_results:
                with open(os.path.join(save_path, path.split('/')[-1]), 'rb') as f:
                    result = cPickle.load(f)
                num_instance = result['pred_RTs'].shape[0]
            ...
            if draw:
                ...
                path = dataloder.dataset.test_img_list[i].replace('rgb', 'labels').replace('.png', '_label.pkl')
                with open(path, 'rb') as f:
                    data = cPickle.load(f)
                image_path = dataloder.dataset.test_img_list[i]
                image_path_parsing = image_path.split('/')

                # print(image_path)
                scene_id = int(image_path.split('/')[-3][-1])
                # rgb
                import cv2
                import numpy as np
                ...
                for intrinsics_path in  dataloder.dataset.test_intrinsics_list:
                    if block in intrinsics_path:
                        intrinsics = np.loadtxt(intrinsics_path).reshape(3, 3)
                        break

                # cat_id = result['gt_class_ids'].tolist().index(2) #bottle
                cat_id = result['gt_class_ids'].tolist().index(2)
                # cat_id = np.where(result['gt_class_ids'] == 1)[0]
                ...
                #draw single 
                # draw_single_detections(cat_id,scene_id,image, os.path.join(save_path, 'draw'), 'real_test', image_path_parsing[-2]+'_'+image_path_parsing[-1], intrinsics,
                #                     result['gt_bboxes'], None, None, result['gt_RTs'], result['gt_scales'],
                #                     result['pred_bboxes'], result['pred_class_ids'], None, result['pred_RTs'], None, result['pred_scales'],
                #                     draw_gt=True, draw_pred=True)
