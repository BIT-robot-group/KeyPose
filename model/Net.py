...
class Net(nn.Module):
    ...
    def forward(self, inputs):
        ...
        batch_kpt_query, heat_map = self.IAKD(rgb_local, pts_local,rgb_global,pts_global)

        
        atten_feat = torch.cat([pts_global.repeat(1, 1, npoint), pts_local, rgb_global.repeat(1, 1, npoint), rgb_local], dim=1)

        kpt_3d = torch.bmm(heat_map, pts)


        # kpt_feature = torch.bmm(heat_map, torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2))
        kpt_feature = torch.bmm(heat_map, atten_feat.transpose(1, 2))

        # kpt_feature = self.GAFA(kpt_feature, kpt_3d.detach(), torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2), pts)
        kpt_feature = self.GAFA(kpt_feature, kpt_3d.detach(), atten_feat.transpose(1, 2), pts)
...
class Loss(nn.Module):
    ...
    def forward(self, endpoints):
        ...
        # loss_all = self.cfg.pose*loss_pose + self.cfg.cd*loss_cd + \
