...
class TestDataset():
    def __init__(self, image_size, sample_num, data_dir, setting, dataset_name):
        ...
        if dataset_name == 'real':
            self.intrinsics = [591.0125, 590.16775, 322.525, 244.11084]
            result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_result', 'REAL275', 'results_*.pkl'))
            # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_results', 'REAL275', 'results_*.pkl'))
            # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_results_refine_for_mug', 'REAL275', 'results_*.pkl'))
            
        elif dataset_name == 'camera':
            self.intrinsics = [577.5, 577.5, 319.5, 239.5]
            result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_result', 'CAMERA25', 'results_*.pkl'))
            # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_results', 'CAMERA25', 'results_*.pkl'))
            # result_pkl_list = glob.glob(os.path.join(self.data_dir, 'segmentation_results_refine_for_mug', 'CAMERA25', 'results_*.pkl'))
