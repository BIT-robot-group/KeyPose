...
def draw_bboxes(img, img_pts, color,dash=False):
    ...
    if not dash:
        ...
        color_ground = (int(color[0]*0.3), int(color[1]*0.3), int(color[2]*0.3))
        for i, j in zip([4, 5, 6, 7], [5, 7, 4, 6]):
            img = drawline(img, tuple(img_pts[i]), tuple(img_pts[j]), color_ground, 2)
        # draw pillars in minor darker color
        color_pillar = (int(color[0]*0.6), int(color[1]*0.6), int(color[2]*0.6))
        for i, j in zip(range(4), range(4, 8)):
            img = drawline(img, tuple(img_pts[i]), tuple(img_pts[j]), color_pillar, 2)
        # draw top layer in original color
        for i, j in zip([0, 1, 2, 3], [1, 3, 0, 2]):
            img = drawline(img, tuple(img_pts[i]), tuple(img_pts[j]), color, 2)
...
def draw_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size, pred_class_ids,
    ...
    """
    out_path = os.path.join(out_dir, '{}_{:.4}_pred.png'.format(data_name, str(img_id)))
    ...
    if draw_gt:
        for i in range(gt_sRT.shape[0]):
            ...
            transformed_bbox_3d = transform_coordinates_3d(bbox_3d, sRT)
            projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
            img = draw_bboxes(img, projected_bbox, (0, 255, 0),False)
