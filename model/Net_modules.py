...
from model.gm import graphmodule
import random
...
def Cosine(vec1, vec2) :
    result = InnerProduct(vec1,vec2) / (torch.norm(vec1, p=2, dim=2, keepdim=True)  * torch.norm(vec2, p=2, dim=1, keepdim=True) )
    return result

def VectorSize(vec) :
    return torch.sqrt(sum(torch.pow(v,2) for v in vec))

def InnerProduct(vec1, vec2) :
    # return sum(torch.bmm(v1,v2) for v1,v2 in zip(vec1,vec2))
...
def Euclidean(vec1, vec2) :
    ...
    print(torch.norm(vec1.unsqueeze(2)-vec2.unsqueeze(1),3).shape)
    # return math.sqrt(sum(math.pow((v1-v2),2) for v1,v2 in zip(vec1, vec2)))
    return torch.sqrt(sum(torch.pow(torch.norm(vec1.unsqueeze(2)-vec2.unsqueeze(1),3),2)))

def Theta(vec1, vec2) :
    return torch.acos(Cosine(vec1,vec2)) + torch.deg2rad(torch.Tensor([10]).cuda())

def Triangle(vec1, vec2) :
    theta = torch.deg2rad(Theta(vec1,vec2))
    # return (VectorSize(vec1) * VectorSize(vec2) * torch.sin(theta)) / 2
    return (torch.norm(vec1, p=2, dim=2, keepdim=True)  * torch.norm(vec2, p=2, dim=1, keepdim=True)  * torch.sin(theta)) / 2
...
def Sector(vec1, vec2) :
    ...
    ED = torch.cdist(vec1, vec2, p=2.0)
    # print(ED.shape)
    MD = Magnitude_Difference(vec1, vec2)
    theta = Theta(vec1, vec2)
    return torch.pi * torch.pow((ED+MD),2) * theta/360

def TS_SS(vec1, vec2) :
    print(vec1.shape)
    print(torch.norm(vec1, p=2, dim=2, keepdim=True).shape)
    print(vec2.shape)
    print(torch.norm(vec2, p=2, dim=1, keepdim=True).shape)
...
class GAFA_block(nn.Module):
    ...
    def forward(self, kpt_feature, kpt_3d, pts_feature, pts):
        ...
        kpt_feature = self.fuse_mlp(torch.cat([kpt_feature.transpose(1, 2), kpt_global.transpose(1, 2).repeat(1, 1, kpt_num),
                                                pos_enc_l.transpose(1, 2)], dim=1))
...
class AttnBlock(nn.Module):
    ...
    def forward(self, kpt_query, input_feature):
        ...
        # print(kpt_query2.shape) torch.Size([32, 96, 512])
...
class InstanceAdaptiveKeypointDetector(nn.Module):
    def __init__(self, cfg):
        ...
        self.in_conv = nn.Conv1d(512, 512, kernel_size=1, stride=1)
        self.out_conv = nn.Conv1d(512, 512, kernel_size=1, stride=1)
    ...
    def forward(self, rgb_local, pts_local,rgb_global,pts_global):
        """_summary_
        ...
        input_feature = torch.cat((pts_global.repeat(1, 1, n), pts_local, rgb_global.repeat(1, 1, n), rgb_local), dim=1)  # (b, 2c, n)
        ...
        # print(attn.shape)
        # print(batch_kpt_query.shape)

        head = b
        query = self.in_conv(batch_kpt_query.permute(0, 2, 1).contiguous())
        query = query.permute(0, 2, 1).contiguous()
        query = self.norm(query).permute(0, 2, 1).contiguous()
        # key = input_feature
        # value = input_feature
        ...
        for i in range(self.N):
            ...
            idx = random.sample(range(n), self.kpt_num)
            keys.append(input_feature[:,:,idx])
            values.append(input_feature[:,:,idx])

        h_p = w_p=1
        attns_list = []
        for key in keys:
            key = F.adaptive_max_pool1d(key, [1], return_indices=False)
            
            key = self.in_conv(key)
            key = key.permute(0, 2, 1).contiguous()
            key = self.norm(key).permute(0, 2, 1).contiguous()
            key = key.contiguous().view(
                head, self.head_dim,   h_p, w_p
            )  # [head,c,h,w].
            attn_list = []
            for k in key:
                attn = F.conv2d(F.pad(query.unsqueeze(0).permute(0,2,1,3).contiguous(), pad), k.unsqueeze(0))  # [1,1,h,w]
        ...
        # score normalization
        ##################################################################################
        attns = attns * float(512 * h_p * w_p) ** -0.5  # scaling
        attns = torch.exp(attns)  # [head,n,h,w]
        attns_sn = (
            attns / (attns.max(dim=2, keepdim=True)[0]+ 1e-7).max(dim=3, keepdim=True)[0]
        )
        ...
        for idx, value in enumerate(values):
            value = F.adaptive_max_pool1d(
                value, [1], return_indices=False
            )
        ...
        # print(feats.shape) torch.Size([32, 512, 32, 96])
        ...
        # print(feats.shape)
        feats = self.out_conv(feats).permute(0,2,1)

        batch_kpt_query = batch_kpt_query + self.dropout2(feats)
        ...
        # mean1 = torch.mean(batch_kpt_query, 2, keepdim=True)  
        # mean_n1 = torch.norm(mean1, p=2, dim=2, keepdim=True) 
        # mean2 = torch.mean(input_feature, 1, keepdim=True)  
        # mean_n2 = torch.norm(mean2, p=2, dim=1, keepdim=True) 

        heatmap = torch.bmm(batch_kpt_query, input_feature) / (norm1 * norm2 + 1e-7)

        # heatmap = TS_SS(batch_kpt_query, input_feature)
        # print(heatmap.shape) torch.Size([32, 96, 1024])

        # heatmap = torch.bmm(batch_kpt_query-mean1, input_feature-mean2) / ((norm1-mean_n1) * (norm2-mean_n2) + 1e-7)
